...
> 历史记录：下文保留 v0.1.0 当时的打包测试事实。v0.1.1 起项目发布包已停止提供，旧手工附件与 Actions 项目发布包已删除；这些历史测试不代表现在仍提供下载。当前验证见 [v0.1.1 修订报告](V0.1.1_REPORT.md)。
...
| Windows 开发启动 | `npm.cmd run dev -- --strictPort` 成功；首页/移动视口与透明 PNG→JPG 两项测试通过，共 16.5 秒。修复首次依赖刷新后，以 `npm.cmd run dev -- --force --strictPort` 强制重建预打包缓存，再跑完整 14 项全部通过，约 1.6 分钟。另在保留转换结果时运行资源准备脚本，确认服务器存活、没有刷新、结果保留 |
...
7. 初次开发环境复测时文件队列被开发热刷新清空，导致用例等待转换按钮超时。随后从 Vite 日志确认 Worker 首次导入 `pdf-lib` 会触发依赖优化与刷新；添加显式预打包，并排除生成清单和 Service Worker 的监听。强制重建预打包缓存后完整 14 项开发测试通过。生产构建没有 HMR。
