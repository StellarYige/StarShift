...
- Location: tests\matrix\lifecycle.spec.ts:76:1
...
```ts
  ...
  74  | });
  75  |
  76  | test('conversion, clear, retry and tool switching lifecycle rounds', async ({ page, browserName, browser }, info) => {
...
> 109 |     else { expect(browserName).not.toBe('chromium'); await expect(page.locator('.notice').last()).toContainText('不兼容'); }
  ...
  117 |   await evidence(info, 'lifecycle.json', { rounds, samples, browserVersion: browser.version(), channel: info.project.use.channel || 'bundled' });
  118 | });
  119 |
```
