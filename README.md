...
[在线使用](https://stellaryige.github.io/StarShift/) · [下载静态发布包](https://github.com/StellarYige/StarShift/releases) · [测试记录](docs/TEST_REPORT.md) · [引擎选型与限制](docs/ENGINE_EVALUATION.md)
...
完整浏览器转换测试另需 Python 3.11+ 和 Chromium：
...
从 [Releases](https://github.com/StellarYige/StarShift/releases) 下载 `StarShift-0.1.0-static.zip`，保留其中的 `engine/`、`fonts/`、`office/`、`pdfjs/`、`licenses/` 和 Service Worker 等所有文件。

默认部署路径是 `/StarShift/`。例如将解压内容放在 `webroot/StarShift/`，在 `webroot` 目录执行：

```powershell
python -m http.server 8080
```

然后访问 `http://localhost:8080/StarShift/`。生产环境使用 HTTPS。不能通过双击 `index.html` 或 `file://` 使用。
...
`dist/` 是完整站点，`release/` 是 ZIP 与 SHA-256 文件。静态包与在线版没有功能差异。
...
现有仓库是 `StellarYige/StarShift`，默认分支 `main`。`.github/workflows/pages.yml` 会依次执行类型检查、单元测试、构建、Chromium 实际转换检查、打包及 Pages 部署。PR 只检查，不部署。

仓库 `Settings → Pages → Build and deployment → Source` 应为 **GitHub Actions**。Actions 同时保存静态发布包和测试证据；下载公开 Releases 中的静态包无需 GitHub 登录。
...
- 文档引擎加中文字体约 **266 MiB（解压后）**，首次使用需要加载；实际网络传输量由静态服务器压缩决定。建议桌面最新版 Chrome / Edge。尚未完成真实 iOS / Android 和 Safari / Firefox 的文档引擎测试。
...
- 不提供离线 PWA 安装保证；离线/内网使用请下载完整静态包并通过本地 HTTP 服务访问。
...
scripts/              离线资源准备、测试样例、打包、独立 PDF 检查
tests/                单元与浏览器端到端测试、自制样例
.github/workflows/    自动检查与 GitHub Pages 部署
```

自有代码采用 [MIT](LICENSE)。第三方库、LibreOffice/ZetaOffice、Qt 与字体分别遵循原许可，详见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md) 和发布包 `licenses/`。引擎源码获取与重构建入口也列于该文件。
