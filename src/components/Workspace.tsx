...
function WorkspaceBatch({ tool, settings, setSettings, onClear }: { tool: ToolId; settings: Settings; setSettings: Dispatch<SetStateAction<Settings>>; onClear: () => void }) {
  ...
  useEffect(() => () => { generation.current++; controller.current?.abort(); controller.current = null; running.current = false; itemsRef.current = []; outputsRef.current = []; previewTrigger.current = null; urls.current.forEach(url => URL.revokeObjectURL(url)); urls.current.clear(); zipRef.current = ''; }, []);
  ...
  function clear() {
    generation.current++; controller.current?.abort(); controller.current = null; running.current = false; itemsRef.current = []; outputsRef.current = []; previewTrigger.current = null;
    thumbnails.dispose();
