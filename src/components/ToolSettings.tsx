...
export default function ToolSettings({ tool, settings, set, disabled }: { tool: ToolId; settings: Settings; set: (value: Partial<Settings>) => void; disabled: boolean }) {
  ...
  return <fieldset className="settings-fields" disabled={disabled}>
    {(image || raster) && <>
      ...
      {settings.format !== 'png' && <label>图片质量 <span className="field-value">{Math.round(settings.quality * 100)}%</span><input aria-label="图片质量" type="range" min="0.1" max="1" step="0.05" value={settings.quality} onChange={e => set({ quality: Number(e.target.value) })} /></label>}
    </>}
    {image && <>
      <div><span className="field-label">尺寸上限（像素）</span><div className="two-fields"><label><input aria-label="最大宽度" type="number" min="0" max="16384" value={settings.width || ''} placeholder="原始宽度" onChange={e => set({ width: Number(e.target.value) })} /><span className="input-caption">宽度</span></label><span>×</span><label><input aria-label="最大高度" type="number" min="0" max="16384" value={settings.height || ''} placeholder="原始高度" onChange={e => set({ height: Number(e.target.value) })} /><span className="input-caption">高度</span></label></div><p className="hint">保持比例缩小，不放大。留空保留原尺寸。</p></div>
    ...
    {pdf && <>
      ...
      <label>页边距（mm）<input type="number" min="0" max="50" value={settings.margin} onChange={e => set({ margin: Number(e.target.value) })} /></label>
      <p className="hint">每张图片一页，按列表顺序排列。透明区域填充为白色。</p>
    </>}
    {raster && <>
      <label>选择页码<input value={settings.pages} placeholder="全部页面，例如 1-3,5" onChange={e => set({ pages: e.target.value })} /></label>
