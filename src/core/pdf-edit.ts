...
export async function assemblePdf(sources: { id: string; bytes: Uint8Array }[], pages: Pick<PageItem, 'sourceId' | 'page' | 'rotation'>[], progress?: Progress) {
  if (!pages.length) throw new Error('请至少选择一页。');
  const docs = new Map<string, PDFDocument>();
  for (const source of sources) docs.set(source.id, await PDFDocument.load(source.bytes, { updateMetadata: false }));
...
export async function imagesToPdf(images: { blob: Blob; width: number; height: number }[], settings: Settings, progress?: Progress) {
  if (!images.length) throw new Error('请至少选择一张图片。');
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    progress?.('正在生成 PDF…', i + 1, images.length);
  }
  return asBlob(await doc.save(), 'application/pdf');
}
